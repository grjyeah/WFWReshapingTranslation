...
class ChineseFormatter:
    ...
    def __init__(self, ollama_url: str = "http://localhost:11434",
        ...
        self.model_options = {
            ...
            "num_gpu_layers": 999,  # 强制卸载所有层到GPU（24GB显存足够运行7B模型）

            # 性能优化参数
            "num_thread": 24,  # CPU线程数（GPU模式下主要用于预处理）
            "num_batch": 512,  # 批处理大小，提升吞吐量
            ...
            "repetition_penalty": 1.15,  # 提高重复惩罚，避免啰嗦
    ...
    def call_ollama(self, prompt: str, max_retries: int = 2, use_stream: bool = True) -> str:
        ...
        Returns:
            模型生成的文本
        """
        # GPU配置已在__init__的self.model_options中设置
        payload = {
            "model": self.model_name,
            "prompt": prompt,
            "stream": use_stream,
            "options": self.model_options  # 包含num_gpu: 999和num_gpu_layers: 999
        }
