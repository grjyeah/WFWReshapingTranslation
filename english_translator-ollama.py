...
class EnglishTranslator:
    ...
    def __init__(self, lm_studio_url: str = "http://127.0.0.1:1234",
        ...
        # 模型参数配置（优化为精简书面化输出）
        self.model_options = {
            "seed": seed,  # 随机种子，固定后相同输入产生一致输出，确保术语翻译一致性
            ...
            "stop": ["<thinking>", "</thinking>", "<thought>", "</thought>", "<reasoning>", "</reasoning>",
                     "<reflection>", "</reflection>", "============", "End of", "【结束】"]  # 阻止思考模式输出
        }
    ...
    def call_ollama(self, prompt: str, max_retries: int = 2, use_stream: bool = True) -> str:
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You are a professional translator. IMPORTANT: Never use thinking mode or output any <thinking> tags. Translate directly without showing your thought process."
                },
                        ...
                        if 'choices' in data and len(data['choices']) > 0:
                            ...
                            if 'content' in delta:
                                ...
                                if content:  # content可能为None
                                    # 实时过滤思考标签
                                    content, in_thinking_mode = self._filter_thinking_tags(
                                        content, in_thinking_mode
                                    )
    ...
    def _filter_thinking_tags(self, content: str, in_thinking_mode: bool) -> tuple:
        """
        实时过滤思考标签内容

        Args:
            content: 当前生成的内容片段
            in_thinking_mode: 是否处于思考模式中

        Returns:
            (过滤后的内容, 是否仍处于思考模式)
        """
        thinking_tags = ['<thinking>', '<thought>', '<reasoning>', '<reflection>']
        closing_tags = ['</thinking>', '</thought>', '</reasoning>', '</reflection>']
        ...
        while remaining:
            ...
            if not in_thinking_mode:
                ...
                if earliest_pos != -1:
                    # 保留开始标签之前的内容
                    result += remaining[:earliest_pos]
                    # 跳过开始标签
                    remaining = remaining[earliest_pos + len(earliest_tag):]
        ...
        return result, in_thinking_mode
